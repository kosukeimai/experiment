bNegBinMixed <- function(formula, formulaR, grp, data =
                         parent.frame(), coef.start = 0, 
                         sig2.start = 1, Phi.start = 1, p.mean = 0,
                         p.prec = 0.01, a0 = 1, b0 = 1,
                         p.df = 1, p.scale = 1, varb = 0.001,
                         vars = 0.001, varg = 0.001,
                         n.draws = 5000) {
  ...
  if (length(varb) == 1)
    varb <- rep(varb, n.beta)

  if (length(vars) != 1)
    stop("Incorrect input for vars")

  if (length(varg) == 1)
    varg <- rep(varg, n.gamma)
  ...
  res <- .C("R2bnegbinMixedMCMC", as.integer(Y), as.double(X),
            ...
            as.double(a0), as.double(b0), as.integer(p.df),
            as.double(p.scale), as.double(varb), as.double(vars),
            as.double(varg), as.integer(n.draws),
  ...
  return(list(beta = matrix(res$betaStore, byrow = TRUE, ncol = n.beta),
              ...
              accept = res$counter/n.draws,
              acceptg = res$counterg/n.draws))
}
